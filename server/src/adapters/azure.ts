...
export class AzureAdapter implements ProviderAdapter {
  ...
  translateRequest(request: AnthropicMessagesRequest, context: ProviderAdapterContext): TranslatedRequest {
    ...
    // Extract deployment name from model config or use as-is
    const deploymentName = modelConfig.deploymentName || upstreamModelId;
    ...
    for (const msg of request.messages) {
      if (msg.role === 'user') {
        ...
        if (msg.content) assistantMsg.content = this.convertContent(msg.content);
        if (msg.tool_uses) {
    ...
    return {
      url: this.buildUrl(context, deploymentName),
  ...
  translateResponse(response: any, context: ProviderAdapterContext): AnthropicMessagesResponse {
    ...
    if (!choice) throw new Error('No response from Azure');
  ...
  normalizeModelId(modelId: string): string {
    return modelId.replace(/^azure\//, '');
  }

  buildUrl(context: ProviderAdapterContext, deploymentName: string): string {
    ...
    const apiVersion = context.config.apiVersion || '2024-02-15-preview';
